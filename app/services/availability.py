...
async def get_dealership_or_404(db: AsyncSession, dealership_id: int) -> Dealership:
...
async def get_service_type_or_404(
    db: AsyncSession, service_type_id: int
) -> ServiceType:
...
async def check_slot(
    ...
    return await free_qualified_technician_exists(
        db, dealership_id, service_type_id, start, end
    ) and await free_bay_exists(db, dealership_id, start, end)
...
async def list_free_slots(
    ...
    dealership = await get_dealership_or_404(db, dealership_id)
    service_type = await get_service_type_or_404(db, service_type_id)
    ...
    for start, end in candidate_slots:
        if await check_slot(db, dealership_id, service_type_id, start, end):
...
async def check_start_time(
    ...
    service_type = await get_service_type_or_404(db, service_type_id)
    await get_dealership_or_404(db, dealership_id)
